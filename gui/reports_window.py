...
class ReportsWindow:
    ...
    def setup_ui(self):
        ...
        self.notebook.add(self.sales_frame, text=_('sales_reports'))
        self.setup_sales_reports()
        
        # Inventory Reports tab
        self.inventory_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.inventory_frame, text=_('inventory_reports'))
        self.setup_inventory_reports()
        
        # Product Performance tab
        self.performance_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.performance_frame, text=_('product_performance'))
        self.setup_performance_reports()
        
        # Financial Summary tab
        self.financial_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.financial_frame, text=_('financial_summary'))
        self.setup_financial_reports()
    
    def setup_sales_reports(self):
        """Setup sales reports tab"""
        # Control panel
        control_frame = ttk.LabelFrame(self.sales_frame, text=_('sales_report_options'), padding="10")
        control_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=5, pady=5)
        control_frame.columnconfigure(3, weight=1)
        
        # Date range selection
        ttk.Label(control_frame, text=_('from_date')).grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(control_frame, text=_('to_date')).grid(row=0, column=2, sticky=tk.W, padx=(10, 5))
        ...
        ttk.Label(control_frame, text=_('from_time')).grid(row=1, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(control_frame, text=_('to_time')).grid(row=1, column=2, sticky=tk.W, padx=(10, 5))
        ...
        ttk.Button(quick_dates_frame, text=_('today'), command=self.set_today).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_dates_frame, text=_('yesterday'), command=self.set_yesterday).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_dates_frame, text=_('this_week'), command=self.set_this_week).pack(side=tk.LEFT, padx=2)
        ttk.Button(quick_dates_frame, text=_('this_month'), command=self.set_this_month).pack(side=tk.LEFT, padx=2)
        
        # Action buttons
        actions_frame = ttk.Frame(control_frame)
        actions_frame.grid(row=3, column=0, columnspan=4, pady=10)
        
        ttk.Button(actions_frame, text=_('generate_report'), command=self.generate_sales_report).pack(side=tk.LEFT, padx=5)
        ttk.Button(actions_frame, text=_('export_csv'), command=self.export_sales_csv).pack(side=tk.LEFT, padx=5)
        ttk.Button(actions_frame, text=_('print_report'), command=self.print_sales_report).pack(side=tk.LEFT, padx=5)
        
        # Results area
        results_frame = ttk.LabelFrame(self.sales_frame, text=_('sales_report_results'), padding="5")
        ...
        sales_headings = {
            'Date': _('date_header'),
            'Receipt #': _('receipt_no'),
            'Customer': _('customer'),
            'Items': _('items_count'),
            'Subtotal': _('subtotal'),
            'Tax': _('tax_label').rstrip(':'),
            'Total': _('total'),
            'Payment': _('payment')
        }

        for col, text in sales_headings.items():
    ...
    def setup_inventory_reports(self):
        ...
        # Control panel
        inv_control_frame = ttk.LabelFrame(self.inventory_frame, text=_('inventory_report_options'), padding="10")
        inv_control_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=5, pady=5)
        
        # Report type selection
        ttk.Label(inv_control_frame, text=_('report_type')).grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        # Category filter
        ttk.Label(inv_control_frame, text=_('category')).grid(row=0, column=2, sticky=tk.W, padx=(10, 5))
        self.inv_category_var = tk.StringVar(value=_('all_categories'))
        ...
        ttk.Button(inv_actions_frame, text=_('generate_report'), command=self.generate_inventory_report).pack(side=tk.LEFT, padx=5)
        ttk.Button(inv_actions_frame, text=_('export_csv'), command=self.export_inventory_csv).pack(side=tk.LEFT, padx=5)
        ttk.Button(inv_actions_frame, text=_('print_report'), command=self.print_inventory_report).pack(side=tk.LEFT, padx=5)
        
        # Results area
        inv_results_frame = ttk.LabelFrame(self.inventory_frame, text=_('inventory_report_results'), padding="5")
        ...
        inv_headings = {
            'ID': _('id'),
            'Product': _('product_label'),
            'Category': _('category'),
            'Unit': _('unit'),
            'Stock': _('stock'),
            'Min Stock': _('min_stock'),
            'Cost Price': _('cost_price'),
            'Selling Price': _('selling_price'),
            'Value': _('value')
        }

        for col, text in inv_headings.items():
    ...
    def setup_performance_reports(self):
        ...
        # Control panel
        perf_control_frame = ttk.LabelFrame(self.performance_frame, text=_('performance_report_options'), padding="10")
        perf_control_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=5, pady=5)
        
        # Date range
        ttk.Label(perf_control_frame, text=_('from_date')).grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(perf_control_frame, text=_('to_date')).grid(row=0, column=2, sticky=tk.W, padx=(10, 5))
        ...
        # Sort by
        ttk.Label(perf_control_frame, text=_('sort_by')).grid(row=0, column=4, sticky=tk.W, padx=(10, 5))
        ...
        ttk.Button(perf_actions_frame, text=_('generate_report'), command=self.generate_performance_report).pack(side=tk.LEFT, padx=5)
        ttk.Button(perf_actions_frame, text=_('export_csv'), command=self.export_performance_csv).pack(side=tk.LEFT, padx=5)
        
        # Results area
        perf_results_frame = ttk.LabelFrame(self.performance_frame, text=_('product_performance_results'), padding="5")
        ...
        perf_headings = {
            ...
            'Category': _('category'),
            'Qty Sold': _('qty_sold'),
            'Times Sold': _('times_sold'),
            'Revenue': _('revenue'),
            'Avg Price': _('avg_price'),
            'Last Sold': _('last_sold')
        }

        for col, text in perf_headings.items():
            self.performance_tree.heading(col, text=text)
    ...
    def setup_financial_reports(self):
        ...
        # Control panel
        fin_control_frame = ttk.LabelFrame(self.financial_frame, text=_('financial_summary_options'), padding="10")
        fin_control_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=5, pady=5)
        
        # Period selection
        ttk.Label(fin_control_frame, text=_('period')).grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(self.custom_date_frame, text=_('from_label')).grid(row=0, column=0, padx=(0, 5))
        ...
        ttk.Label(self.custom_date_frame, text=_('to_label')).grid(row=0, column=2, padx=(10, 5))
        ...
        ttk.Button(fin_actions_frame, text=_('generate_summary'), command=self.generate_financial_summary).pack(side=tk.LEFT, padx=5)
        ttk.Button(fin_actions_frame, text=_('export_summary'), command=self.export_financial_csv).pack(side=tk.LEFT, padx=5)
        
        # Results area
        fin_results_frame = ttk.LabelFrame(self.financial_frame, text=_('financial_summary'), padding="10")
    ...
    def create_financial_summary_ui(self, parent):
        ...
        # Sales metrics
        sales_frame = ttk.LabelFrame(parent, text=_('sales_metrics'), padding="10")
        sales_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=5)
        
        self.total_sales_label = ttk.Label(sales_frame, text=_('total_sales') + ': -', font=('Arial', 12, 'bold'))
        self.total_sales_label.grid(row=0, column=0, sticky=tk.W, pady=2)
        
        self.total_revenue_label = ttk.Label(sales_frame, text=_('revenue') + ': -', font=('Arial', 12, 'bold'))
        self.total_revenue_label.grid(row=1, column=0, sticky=tk.W, pady=2)
        
        self.avg_sale_label = ttk.Label(sales_frame, text=_('avg_sale') + ': -')
        self.avg_sale_label.grid(row=2, column=0, sticky=tk.W, pady=2)
        
        self.total_tax_label = ttk.Label(sales_frame, text=_('total_tax') + ': -')
        self.total_tax_label.grid(row=3, column=0, sticky=tk.W, pady=2)
        
        # Payment methods
        payment_frame = ttk.LabelFrame(parent, text=_('payment_methods'), padding="10")
        payment_frame.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        self.cash_sales_label = ttk.Label(payment_frame, text=_('cash_sales') + ': -')
        self.cash_sales_label.grid(row=0, column=0, sticky=tk.W, pady=2)
        
        self.card_sales_label = ttk.Label(payment_frame, text=_('card_sales') + ': -')
        self.card_sales_label.grid(row=1, column=0, sticky=tk.W, pady=2)
        
        self.credit_sales_label = ttk.Label(payment_frame, text=_('credit_sales') + ': -')
        self.credit_sales_label.grid(row=2, column=0, sticky=tk.W, pady=2)
        
        # Top products
        top_products_frame = ttk.LabelFrame(parent, text=_('top_5_products'), padding="5")
        ...
        top_headings = {
            'Rank': _('rank'),
            'Product': _('product_label'),
            'Quantity Sold': _('quantity_sold'),
            'Revenue': _('revenue')
        }

        for col, text in top_headings.items():
            self.top_products_tree.heading(col, text=text)
    ...
    def load_categories(self):
        ...
        try:
            ...
            category_names = [_('all_categories')] + [cat.name for cat in categories]
            self.inv_category_combo['values'] = category_names
            self.inv_category_combo.set(_('all_categories'))
    ...
    def generate_inventory_report(self):
        ...
        try:
            ...
            try:
                ...
                if category_filter != _('all_categories'):
